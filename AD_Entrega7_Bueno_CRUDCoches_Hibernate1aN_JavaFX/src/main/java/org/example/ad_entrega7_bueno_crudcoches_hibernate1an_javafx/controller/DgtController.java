    ...
    void onMultasClick(ActionEvent event) {
        ...
        if (cocheSeleccionado != null) {
            try {
                ...
                /*La siguiente línea obtiene una instancia del controlador de la vista que se está cargando a través
                del FXMLLoader. En otras palabras, fxmlLoader es el objeto que carga la vista de la interfaz gráfica
                (FXML) y, con getController(), se obtiene el controlador asociado a esa vista.*/
                MultasController multasController = fxmlLoader.getController();
                //pasa el coche seleccionado al controlador de multas
                    /*Este código pasa el objeto cocheSeleccionado (que es un objeto de tipo Coches)
                    al método metod() del controlador MultasController.*/
